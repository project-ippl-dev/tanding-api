...
import (
	...
	mock_file "github.com/project-ippl-dev/tanding-api/mocks/file"
	"net/http"
	"testing"
...
type handlerMock struct {
	mockUsecase *mock_file.MockUsecase
}
...
func newHandlerMock(t *testing.T) (handlerMock, *echo.Echo) {
	...
	mockFileUsecase := mock_file.NewMockUsecase(mockCtrl)

	e := echo.New()

	return handlerMock{
		mockUsecase: mockFileUsecase,
	}, e
}
...
func TestHandler_Upload(t *testing.T) {
	...
	fileHandler := file.NewHandler(mock.mockUsecase)
	...
	testCases := []struct {
		...
		},
		{
			...
			testMock: func(c echo.Context, mock handlerMock) {
				c.SetPath(path)
				mock.mockUsecase.EXPECT().ReadMIMEType(gomock.Any(), gomock.Any()).SetArg(1, file.FileInformation{
					FileName: "test",
					Size:     6 * 1024 * 1024,
...
func TestHandler_Base64(t *testing.T) {
	...
	validReq := file.Base64Params{
		Data: "base-64-here",
		Dir:  "photo",
	}

	invalidReq := file.Base64Params{
		Data: "base-64-here",
		Dir:  "",
	}
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedResponse:   "code=400, message=Unmarshal type error: expected=file.Base64Params, got=string, field=, offset=14, internal=json: cannot unmarshal string into Go value of type file.Base64Params",
			expectedErr:        true,
			expectedStatusCode: http.StatusBadRequest,
			testMock: func(c echo.Context, mock handlerMock) {
				c.SetPath(path)
